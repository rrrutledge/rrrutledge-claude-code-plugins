...
- **"Already clear from context" reaches beyond the literal thread — it covers a fact already given to this recipient anywhere, including a linked post or an earlier message in a different conversation with them.** Confirming an action tied to such a detail restates it if repeated; give the confirmation and drop the detail.
