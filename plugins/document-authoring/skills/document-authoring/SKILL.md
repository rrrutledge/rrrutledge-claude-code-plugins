...
- **Adding a new recipient to an existing thread: state the action, don't address them.**
  ...
  **Exception — the new recipient is being added because you're flagging a discrepancy in something they own:** fold the introduction and the direct question into the same message instead of splitting them, per the surfacing-a-discrepancy pattern above ("I agree these folks shouldn't have that access - Jeff, can we tighten it to just the board members?") - the whole reason for adding them is to ask, so a bare no-context "Adding X here" followed by a separate question would just delay the actual point.
...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  These skip-exceptions apply only when **Register** hasn't already called for warmth via the leadership axis — where it has, the close holds regardless of what the message is about, **except when the message's own concrete next step is a live, still-open question you're ending on rather than settled news to close warmly around** (e.g. "Jeff, is it possible to have just the board members with access?") - the open question already is the close, and stacking warmth before it reads as declaring the matter settled when it isn't.
