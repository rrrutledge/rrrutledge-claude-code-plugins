...
- **When a specific factual question went unanswered, restate that exact question** as a direct yes/no rather than a generic check-in; pair it with an explicit release if there's a plausible internal reason for the delay. Shrink to the brief verbal frame plus the pointing-emoji gesture instead when the unanswered question is visible elsewhere in the same thread (see the repeat-nudge bullet above). **Exception — the delay is a promised internal check by the recipient**, per the internal-process bullet below.
...
- **When the thing you're nudging on is stuck in the recipient's own internal process, whether to ask a question depends on whether they promised a specific answer.**
  For a generic status nudge (no specific answer was promised), lead with trust in their follow-through rather than a status question: "I know you're always on top of moving this through" carries the release on its own, so drop the separate no-pressure line. Reaching out at all is what reminds them — the nudge doesn't need an explicit question to land, so a plain acknowledgment or even a wordless reaction can do the job. On a first touch a bare reaction is often enough; hold a worded reply in reserve for the next round, once a second touch is actually due.
  When they instead promised to come back with a specific answer ("I'll check with our team and let you know"), a plain, greeted "just checking if you had a chance to look" is the honest register with a warm, established contact — this is the one case where "just checking in" earns its keep despite the general ban on it above, since the recipient's own process, not indifference, explains the silence.
