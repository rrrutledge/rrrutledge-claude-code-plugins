...
- **When someone mentions something negative, match your response to its role in the message:**
  ...
  - **A stated constraint or caveat** (e.g. "budget climate is tough"): release the pressure plainly ("Either way is fine") and stop; don't echo the framing back.
    ...
    **Exception — a hedge on the odds of an outcome someone is actively pursuing for you, not a constraint on their own capacity** (e.g. "this would be a low probability event" about a favor already in motion): echo the hedge back plainly ("As you say, likely not") rather than releasing the pressure and stopping. Accepting long odds together on a still-open favor is candor between friends, not pressure — unlike a declined-favor excuse, it doesn't put them on the spot, since the outcome was never theirs to control; and unlike a frustration or discouraging update on something already concluded (below), there's no settled failure being rehashed, just shared realism about what's still in motion.
