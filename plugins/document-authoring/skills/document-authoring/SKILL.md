...
- **When someone was specifically named or thought of for a role, say that plainly instead of describing the general need it fills.**
  "You got nominated as a moderator for the APAC leg" reads as a personal invitation; "we're lining up moderators and are light on APAC coverage, would you be interested" reads as a form letter even when the underlying ask is identical.
...
- **First-touch outreach: keep the ask to one low-commitment thing, and warm the reference to their past work.**
  ...
  **A warm-intro request reuses a fixed phrasing — "Do you know [person] well enough to make an introduction?" — paired with the standard no-pressure release** (see **Core voice**'s explicit-release rule).
...
- **Cold outreach for a commitment: open with the soft ask, not the hard one.**
  ...
  The same softening governs pivoting an already-warm thread toward a commitment: keep it to one open "is that something we could explore together?" and cut both the concrete call-to-action and the list of benefits — then close with a no-pressure release per **Core voice**, same as any other ask. This case keeps that release even though its own ask is an open question, unlike Core voice's own carve-out for a fresh, standalone open question — a pivot is turning the thread's ongoing subject into a commitment, not raising something new.
