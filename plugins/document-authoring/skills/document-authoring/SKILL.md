...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  These skip-exceptions apply only when **Register** hasn't already called for warmth via the leadership or peer-relationship axis — where it has, the close holds regardless of what the message is about. A first follow-up after an initial live call with a forming peer contact is this case: the concrete-next-step skip doesn't apply just because the message also names a minor logistics detail (e.g. "I'll send the ticket link once it's up") rather than a scheduled call or dated follow-up.
