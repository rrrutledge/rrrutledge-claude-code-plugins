...
- **A thank-you for a small favor is a name and an exclamation — nothing else.**
  ...
  In email, this collapses the whole reply to that one line - the name inside "Thanks, Dave!" is both the address and the close, so it replaces the standalone "Hi Name -" greeting and the `Russ` sign-off, not just the body between them. Accepting a connection request or a similarly small, already-fulfilled ask counts as the "small favor" this applies to. When they close with an offer to help further ("Happy to help however I can"), a reply reacting to it ("Will do.") still counts as the restatement Brevity already bans, even though it directly answers something they just said rather than volunteering an unprompted next step - the favor itself is already done, so nothing more needs transacting in writing right now.
...
- **Email sign-off**: just `Russ` on its own line — no valediction before it, and no closing word fused onto that same line (no "Thanks, Russ", "Best, Russ", or similar; the line is `Russ` alone, full stop). A genuine thanks belongs in the body as its own sentence (see **Light, genuine appreciation**), not welded onto the sign-off — and skip it entirely when there's nothing yet to thank the recipient for, e.g. a first-touch ask where they haven't done anything for you yet.
  **A third exception: a small-favor thank-you that collapses to one line** (see Brevity's "A thank-you for a small favor is a name and an exclamation") **skips the sign-off too** - the name inside "Thanks, Dave!" is already the close, so there's no separate `Russ` line under it.
