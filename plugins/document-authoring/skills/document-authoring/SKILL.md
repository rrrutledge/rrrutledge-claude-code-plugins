...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  A next step that's only conditional on someone else's action (e.g. "once the rest sign off") isn't dated, but it still earns a forward-looking add-on - name the concrete outcome that action unlocks, with the same unhedged energy as any other close. The note drops out only once the next step is itself already concrete and dated.
