...
- **A clause split out of a run-on, or a sign-off line that echoes something the correspondent already said, still counts as restatement.**
  ...
  **Exception - when the whole reply is otherwise a bare acknowledgment with nothing else to add ("Thanks.")**, reciprocating their wish ("You too!") is the reply's content, not restatement padding tacked onto a fuller message. Keep it, and let a separate forward-looking close drop with it, since the bare acknowledgment-plus-reciprocation already reads as the whole close.
