...
- **A repeat nudge on an already-unanswered ask shrinks further than the first** — drop the warm-up context line and any separate reassurance clause, and ask one direct binary question with the release folded into the question itself rather than tacked on after.
  ...
  **When the repeat also carries genuine new content** (a lower-commitment alternative, fresh information), lengthen that same frame to name the new material alongside the gesture.
  This is distinct from having sent the person several emails on **different** threads recently — that pattern gets its own brief acknowledgment instead, not a shrink (see below).
- **When you've sent this person several emails recently across different topics, acknowledge the volume before the ask.** A quick "apologies for so many emails" upfront reads as considerate rather than nagging, when the pattern spans multiple threads rather than repeating on this one.
...
- **When someone is the last of several people completing the same step, frame the close around the group's completion, not your own want.** Naming what finishing unblocks for the whole group reads as gratitude, not a personal push for your own timeline.
