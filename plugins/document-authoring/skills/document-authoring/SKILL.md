...
4. **Stage, never send** — a draft that survives verify is staged for Russell's approval; by default he sends it himself.
   ...
   **LinkedIn text is the standing case of this** - never automated by policy, so it's always handed over in chat rather than staged in the composer - but that only changes delivery: steps 1-3 (Read, Write, Verify) still run on it in full before it reaches Russell, the same as any other message.
