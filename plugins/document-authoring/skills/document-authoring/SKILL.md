...
- **Forwarding a colleague's message via DM or chat:** paste the bare permalink on its own line rather than anchoring it to a paraphrase, so the platform's own unfurl shows the original message in full - anchoring it to summary text buries the very thing the recipient needs to see.
...
- **Relaying a request that originated with someone else: attribute it to them, not to yourself.** Phrase it as "[Name] was asking if..." rather than presenting their ask as your own personal request, so the recipient knows whose ask they're responding to.
...
- **1:1s often open with no greeting at all, or the person's name.**
  **In an active, frequently-used 1:1 thread, skip a fresh warm opener even when raising a new topic, not only when replying to their last message** - open with a terse contextual acknowledgment when replying ("Ah, got it.") or a bare "Hi [Name] -" running straight into the new ask, instead of a named warm greeting plus explicit thanks ("Hey Olive, thanks for the nudge on this") or a separate Open-with-warmth line - the existing thread already carries the context, so a fuller opener reads as over-warm.
...
- **Close nearly any ask, offer, or unsolicited handoff with an explicit no-pressure release** — "If not, then it's fine too - no problem." / "Either way is fine." — so saying no, or not using what you sent, costs the recipient nothing. This is a default habit, not special to money or a formal commitment.
  **Skip the separate closing release when the ask itself already opens on a conditional** ("if you have time," "if you get a chance") - that framing already carries the release, the same way a soft commitment carries its own limits without a trailing disclaimer (see **A soft commitment already carries its own limits** above).
...
- **Thanking a volunteer, community contributor, sponsor or partner, or someone Russell leads for real completed work or committed support is the exception — warm and effusive is in-voice**, emoji and exclamation marks included ("Thank you for being with us 🙏🙏!!!!", "Sounds good 👏🎉‼️ Thank you for working on it!"). This covers people giving their time freely and staff/team members reporting to him on work that mattered — not paid vendors, arms-length colleagues, or routine replies. A committed sponsor or partner is a warm partner here, not an arms-length counterparty: when they commit or come through, run the gratitude hot and personal ("That's amazing! You have been such a support…"), name the specific things they've done for the community across the relationship (this and prior) rather than an abstract thanks, and speak in the first person.
