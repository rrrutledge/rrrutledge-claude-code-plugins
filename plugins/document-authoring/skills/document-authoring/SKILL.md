...
- **Nudging a prospect on a stalled commitment decision (a sponsorship, a sale) asks for the outcome, not receipt** - "let us know what you found," since what's pending is their decision, not confirmation the materials arrived. Lead with a peer already committed as the fresh-reason hook, the same lever as the existing-contributor nudge above, and keep the offer-to-help close instead of cutting it, since a real money decision may genuinely need more information before it lands.
