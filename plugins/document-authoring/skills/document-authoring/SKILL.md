...
- **Don't re-paste something you already shared upthread** — a link, file, or detail the recipient already has.
  ...
  **That exception itself doesn't apply when the whole reason for writing is that they're just back from an OOO/auto-reply** — the return is reason enough on its own, and the quoted original sitting right below already carries the ask for their first real read. Keep that resurface to the trigger plus a warm close; don't restate the ask or re-paste the link.
