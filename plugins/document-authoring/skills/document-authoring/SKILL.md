...
- **1:1s often open with no greeting at all, or the person's name.**
  **Replying to someone who just messaged in an active 1:1 thread, open with a terse contextual acknowledgment of what they said** ("Ah, got it.") instead of a named warm greeting plus explicit thanks ("Hey Olive, thanks for the nudge on this") — the existing thread already carries the context, so a fuller opener reads as over-warm for a reply that's continuing a conversation already in progress.
...
- **Email sign-off**: just `Russ` on its own line — no valediction before it, and no closing word fused onto that same line (no "Thanks, Russ", "Best, Russ", or similar; the line is `Russ` alone, full stop). A genuine thanks belongs in the body as its own sentence (see **Light, genuine appreciation**), not welded onto the sign-off — and skip it entirely when there's nothing yet to thank the recipient for, e.g. a first-touch ask where they haven't done anything for you yet.
  ...
  **Within an active, same-day back-and-forth with a contact Register already places in the warm axis - each side answering within minutes or hours - skip the `Russ` line too.** The exchange reads as a live conversation rather than a formal note, the same register shift chat composers already get.
...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  Skip the note when sharing news into someone else's channel or group, when the message already states a concrete next step (a follow-up date), or when the note would restate something the recipient hasn't agreed to yet - see the Asks "when a contact declines" rule; a plain thanks or "no worries" still closes the message fine.
  **Keep the close when that already-stated next step is a call or meeting time being locked in, referencing the confirmed occasion lightly** ("See you then!") **rather than a generic forward-looking phrase or a full restatement of the date/time.**
