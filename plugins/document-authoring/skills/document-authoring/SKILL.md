...
2. **Write** — draft the message.
   ...
   **When one subject-line email thread has forked into several loosely related exchanges** (a sponsorship logo, a payment status, a separate ask, all under one "Re: Event Name" subject), anchor on the single **literal most recent message in the whole thread**, not a narrower sub-branch chosen because it happens to match this message's specific topic — Russell treats the whole subject as one ongoing conversation. It's normal for him to fold another live topic from the thread into the same reply rather than sending a separate single-topic message for each.
