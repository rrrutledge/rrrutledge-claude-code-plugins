...
- **A person referenced but not being addressed, or not on that platform** — link their name to their profile/permalink on the same platform, otherwise to their LinkedIn profile. Skip the link when the recipient already personally knows the person referenced (a colleague, someone on their own team) - the link is for a reader who might need to look the person up, not a redundant courtesy to someone who already works with them.
...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  This extends to nudging on someone else's stated commitment to connect you with a third party: "any update on connecting with X you mentioned" still presumes the connection is already in motion - recast it as a plain forward request instead, "please let me know if there's a chance to talk with X", which drops the presumption entirely rather than softening it.
