...
- **1:1s often open with no greeting at all, or the person's name.**
  ...
  **This skip is scoped to staying on the same channel - it doesn't carry over when the reply opens a new one.** The first email to someone you've so far only exchanged messages with elsewhere (LinkedIn, Slack, a mutual intro) is a first touch on that channel even though the relationship itself is already warm, and keeps the normal named greeting ("Hi Nathen, ...") rather than dropping to the bare acknowledgment.
