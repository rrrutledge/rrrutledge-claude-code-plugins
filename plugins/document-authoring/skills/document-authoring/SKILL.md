...
- **Answer the question first.**
  ...
  **Exception — a tangential personal aside with no question and nothing to act on** (a passing "hope you're well", a remark about how a call wrapped up, a floated "maybe we could do X together sometime" tossed off as light enthusiasm rather than a real proposal): it doesn't need its own acknowledgment when you're replying to a separate, substantive ask in the same thread. Answering the ask is the whole reply.
...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  When a concrete, already-known future encounter exists (an upcoming event you'll both be at), anchor the close there ("Looking forward to seeing you at the summit!") rather than an enthusiasm line about an outcome that hasn't happened yet (how their pitch, submission, or plan turns out).
