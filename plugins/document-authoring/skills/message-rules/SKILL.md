...
**A threaded reply drops a link, file, or detail already visible to its reader** — the auto-quoted original sitting below an email reply, or the same Slack/Teams thread a named third party is already watching. Even when the reply is also meant to flag that content for the third party, a bare acknowledgment to the original poster is the whole reply, since the watcher sees the thread's content themselves. This also covers a first reply on a new sub-topic that isn't itself a nudge (e.g. answering a delay explanation elsewhere in the thread). See **Follow-up nudges**' "Don't re-paste" for the repeat-ask case this generalizes.
