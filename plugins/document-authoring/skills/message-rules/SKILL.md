...
- **Soften a flaw you're flagging in someone else's work into a suggestion, not a defect.** "Website would be good to point to X" reads better than "the website link is broken" when the material belongs to a teammate — say what you'd change, skip diagnosing what's wrong with it.
  **When a review turns up several flaws, name only the one that actually blocks moving forward and call the rest good to go anyway** — approving despite the unfixed secondary issues reads as trusting the recipient to sort them in their own time, where holding up otherwise-ready work over every observed flaw reads as gatekeeping.
