...
- **If you have options to resolve, pick the one you'd recommend and ask about that** — let the reader counter if they disagree.
  ...
  - **Exception - when Russell genuinely has no preference between two people, himself included, taking an action, and either is equally positioned to do it:** pose it as an open two-way question rather than resolving it to a single recommended ask. Neither option is actually his lean here, unlike the default above, so both stay live and the reader picks whichever they think fits.
...
- **A status-check nudge into a stalled thread skips Core voice's warmth opener.** The fact or trigger that prompted the check leads the message, with no credit or appreciative line in front of it, since this nudge isn't a reply to something the other person just said or did.
