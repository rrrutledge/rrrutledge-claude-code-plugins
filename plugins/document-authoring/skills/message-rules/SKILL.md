...
Five registers reach outside the palette, and most also stack past the one-emoji cap; keep their emoji as sent, since the cap and the palette bind ordinary replies, not these registers:
...
- **A community-wide call for contributions in ISC Slack** — closing on the custom `:isc:` brand emoji stacked with `:rocket:` is in-voice there, past both the palette and the one-emoji cap, but optional, since an earlier call went out with no emoji at all.
