...
- **Keep a short message plain** — a quick reply is one or two plain sentences, not a structured block.
  ...
  **Exception — a short list of edit requests on the same document** (a placeholder to fill in, a wording fix, a tagging tweak), **one connected train of thought** (a finding, its implication, and the step it suggests; or, in a casual reply, an answer plus a related aside and a deferral on the final call), **or several examples backing one shared finding** (three separate emails that all illustrate the same underlying product flaw): fold these into one flowing paragraph rather than one per line or one per bullet, and save a paragraph break for a genuine change of topic.
...
- **Sign off as "Russ" in email — no sign-off in any chat-style composer.**
  Email sign-offs are always `Russ`, never "Russell". Skip the sign-off entirely in Teams, Slack, a text message, and any other bubble-thread surface - a client portal's chat tab reads as chat here even when the thread carries a subject line.
...
- **The first message of a new text thread names the sender in full and gives a reason for asking.**
  A bare phone number carries no identity the way an email's From line or an ongoing chat thread's history does, so the opening sentence introduces him by full name and org ("this is Russ Rutledge from the Canyon Springs Ward"), and the message gives the reason for asking somewhere before it closes - even when the same check-in has already gone out on another channel to the same person. A reply already inside that same text thread carries its own context and skips re-introducing him.
