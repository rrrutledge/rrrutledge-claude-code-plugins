...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  **Money is a touchy, awkward subject — go soft and gentle rather than barreling through the specifics.** State the general need plainly (it's time to renew, we could use the support) and let the amount or the mechanism surface in their reply, instead of restating the dollar figure or naming the specific portal or process yourself — even one already agreed or used before, since leading with all the specifics reads as forward for a subject that calls for going gently.
