...
This holds over email too, ahead of the email-keeps-its-greeting default in **Core voice**'s "1:1s often open with no greeting" bullet, which is for a professional/arms-length email thread, not a personal-friend one — except the greeting word itself: a personal friend over email still gets a bare name-first greeting ("Hey Jeff," rather than a fully dropped "Yup - no problem"), just never the formal "Hi Name -" address form; only a chat-style channel drops the greeting outright.
