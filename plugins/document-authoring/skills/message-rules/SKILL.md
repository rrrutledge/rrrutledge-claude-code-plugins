---
name: message-rules
description: Russell's outward-message artifact rules - register, warmth, asks, links, personas, emoji, sign-offs, nudges - the shared rubric for any Teams, Slack, email, Jira, or Confluence message he sends. Load when drafting or editing an outward message, and when reviewing one against the rules. The companion to authoring-rules, which holds the medium-independent rules.
---
...
**A threaded reply drops a link, file, or detail already visible to its reader** — the auto-quoted original sitting below an email reply, or, in the same Slack/Teams thread, to a named third party who has already said, earlier in the thread, that they're watching for this. A bare acknowledgment to the original poster is the whole reply even when it also lets that third party know the content is there, since they see the thread themselves - but this covers restatement only, not a genuine ask actually directed at the third party. The same drop-the-restatement logic also covers a first reply on a new sub-topic that isn't itself a nudge (e.g. answering a delay explanation elsewhere in the thread). See **Follow-up nudges**' "Don't re-paste" for the repeat-ask case this generalizes.
...
- **If you have options to resolve, pick the one you'd recommend and ask about that** — let the reader counter if they disagree.
  ...
  - **Exception - when someone else already raised or owns the idea, keep it an open two-way question rather than resolving to one recommended ask.**
    Ask whether they still want to carry it through, naming yourself as the fallback only for a genuine timing crunch, since picking one for them upfront would steamroll their initiative.
  ...
  - **Exception — when someone else's message offers you a menu of ways to help, accept every offer that genuinely helps rather than picking just one.** Facing "happy to answer questions/hop on a call/refer you — let me know what makes sense," name the two or three that actually help, stated outright — not a single safe pick, and not handed back with "whichever works for you."
...
- **When a contact declines or gives a discouraging read, accept it humbly and stop** — don't tack on a new ask.
  ...
  When their own decline reads as genuinely wanting this to happen and not just a polite close, and a concrete alternative avenue already exists (an event, a program), close by naming that avenue instead of leaning on personal warmth alone. It asks nothing of them, so it reads as accepting the decline, not as a new ask. Skip this on a decline that reads as merely polite; naming an avenue there is pushy, not gracious.
...
If it carries personal content — asking about family by name, sharing life milestones, the other person sharing personal updates in return — the contact is a personal friend, and the register shifts significantly: drop the salutation entirely, in any form that opens on their name rather than the reaction itself (email keeps a bare name-first form - see below), go much more casual, use emoji freely, and skip structured acknowledgment phrases.
In Slack or Teams, a personal friend gets "Yup - no problem 👍", not "Hi Max - Thanks for the update. I'll look forward to hearing from you next week!"
This holds over email too, ahead of the email-keeps-its-greeting default in **Core voice**'s "1:1s often open with no greeting" bullet, which is for a professional/arms-length email thread, not a personal-friend one — except the greeting word itself: a personal friend over email still gets a bare name-first greeting ("Hey Jeff," rather than a fully dropped "Yup - no problem"), just never the formal "Hi Name -" address form.
**Once a personal-friend email is moving as fast as a chat thread** (several replies back and forth with that friend within the same day), the greeting drops outright too, the same as a chat-style channel - the immediate, in-progress feel that normally only a Slack/Teams thread carries is what licenses dropping it, not the medium itself. (**Core voice**'s "Replying to someone who just messaged" bullet states the email default as unconditional; this fast-thread pace is the one case where a personal-friend email drops the greeting the way that bullet reserves for chat.)

**How warm a close is comes down to one axis: what kind of relationship this is or is becoming — not how long you've known the person, and not their role, rank, or org relative to yours; neither rank nor tenure overrides warmth in either direction, and what matters instead is whether this is a relationship worth investing energy in. Decide it here — this is the single source of truth, and the closing rules elsewhere describe only how to execute, not when.**
...
- **A transactional or arms-length relationship** — HR processing a routine request, a former employer, an adversarial counterparty, a superior you aren't building rapport with → brief and neutral: acknowledge and close with no added enthusiasm, even when the news is good ("glad it worked out" at most, never "This'll be great") and even after a firm no or a long negotiation. Don't recap the answer you were given or praise how they handled it — "Thanks for getting back to me. Sounds good." is the whole reply (execute per **Brevity**).
  - **Keep a superior at a single-word thanks** - never extra positivity (see the superior-address rule below), until warmth is actually established.
  - **Cordial recurring business contact isn't warm rapport on its own** - this covers a community or working-group colleague you deal with only on business (inviting them to speak, coordinating logistics), and repetition alone never promotes it into one — the test is whether a peer relationship is what's actually being built, not how many times you've exchanged mail.
  - **A contact leaves this bucket only on a genuine warmth signal or when you actually lead them** - and that warmth signal includes Russell's own prior sent messages, not only the correspondent's: once he's already welcomed someone with real enthusiasm ("Looking forward to it!", "I'm personally very glad it worked out"), that correspondence has already crossed into the peer bucket, and every later reply carries that same warmth forward (the closing note included). A recurring contact often has warmth on record from an earlier thread on a different subject, even when the thread in front of you now is a fresh, businesslike ask - search prior correspondence with them before defaulting them into this bucket on the strength of one cold-looking thread alone.
  - **Real voluntary work of their own earns the warm treatment anyway** - when they've just contributed real voluntary work of their own (submitting a talk, volunteering real effort) rather than simply responding to or coordinating your ask, that single message earns the warm, praise-the-thing-directly treatment even though the relationship otherwise stays in this bucket (execute per the Core voice thanking-a-contributor rule).
...
- **A clause split out of a run-on, or a sign-off line that echoes something the correspondent already said, still counts as restatement.**
  Splitting a chained sentence per the "own sentence" rule can leave a clause that only restates what the sentence next to it already implies; a closing "you too" line that just hands back a wish the correspondent voiced in the message being answered is the same move. Cut both rather than keeping them just because they're now their own sentence. The same over-split shows up as a bare topic-naming sentence with no content of its own ("The Industry Advisory Board is one specific case.") standing ahead of the sentence that actually says something about it - merge the name and its first fact into one sentence instead of giving the announcement its own empty one.
- **Open a reply on the reaction itself, not a clause re-describing what arrived or what happened.** The thread, the attachment, or the completed action already established the situation, so naming it again before reacting is the same restatement this section already bars: lead straight with the reaction ("Thank you!") and drop the scene-setting clause in front of it.
- **A thank-you for a small favor is a name and an exclamation — nothing else.**
  Don't add a clause explaining why you're grateful — "Thanks so much, Yuki!", not "…really appreciate you taking the time." Reacting to content shared for a favor like that carries the same amplifier caution as authoring-rules' "Say it plainly": a plain, measured line ("is a good story") reads as genuine, not a superlative tied to your own purpose ("is exactly the kind of concrete example I wanted for the talk"), which reads as over-the-top and fake. Reserve real enthusiasm for someone who volunteered unprompted effort, not one you asked for (see **Thanking a volunteer** in Core voice). (How much warmth a close carries is set separately, by relationship — see **Register**.)
...
- **When someone else owns a topic, loop them into the thread rather than answering on Russell's own authority — even when he's confident of the answer.**
  ...
  **Handing an outside contact off to new main contacts keeps Russell's own door open: name the new owners and, in the same sentence, offer his help to everyone on the thread.**
  That offer is an in-voice exception to the helper-tail rule, the same as the warm-partner offer under **Energizing close**, and it stays even though the new owners now handle the day-to-day.
- **Keep asks open and tentative — don't pre-commit.**
  ...
  **A significant or touchy ask — money is one example, a real chunk of someone's time is another — calls for going soft and gentle rather than barreling through the specifics.** State the general need plainly (it's time to renew, we could use a hand with X) and let the amount, the time commitment, or the mechanism surface in their reply, instead of restating the dollar figure, the hours, or the specific portal or process yourself — even ones already agreed or used before, since leading with all the specifics reads as forward for an ask sizable enough to feel like an imposition.
  ...
  When asking a peer to try something you built, frame any wider ask as contingent on how their trial goes, so the message still makes one ask.
  Recast a phrasing that presumes the fact in question already exists - a decision, how and when an update happened, or whether a physical or media deliverable is even ready yet - into one that asks whether it's true at all: "who do we have lined up" becomes "is there anyone we can line up" on a repeated ask, "did Micaela get a chance to sign at yesterday's summit meeting?" becomes "did you hear anything from Micaela?" on a factual status check, and "can you send over that headshot?" becomes "let me know if you have a headshot" on a repeat ask for a photo or document. The presumptive form reads as chasing something already settled or in hand; the open form lets the other person supply whether it's even true yet.
- **First-touch outreach: keep the ask to one low-commitment thing, and warm the reference to their past work.**
  ...
  **Reconnecting after a long gap specifically to ask a favor gets a plain, brief acknowledgment of the imposition before the ask** — "Sorry to just write when I need a favor, but I wanted to ask something."
  **A warm-intro request to a name the contact may not already know earns its ask with real reasoning: what the target leads and why it's relevant, what's in it for the contact too (an expanded network, staying current on the space), and a real credibility link (an org site, an event).** Pair it with the standard no-pressure release (see **Core voice**'s explicit-release rule). When the contact already knows the target, the fixed phrasing alone carries it: "Do you know [person] well enough to make an introduction?" plus that same release.
- **Reporting a completed task, stating what you'll do, sharing a recommendation, or answering a question you researched: lead with the outcome; cut the diagnostic play-by-play and the editorial wrap-up.**
  This also governs the open, ahead of the general **Open with warmth** rule below: skip any channel acknowledgment or praise preamble for how the request arrived, even to someone he works with daily, and lead straight with what was done - distinct from the live-thread terse contextual acknowledgment in the `1on1` persona's "just messaged" opener below, which still applies when actually replying to what they just said; what drops here is naming the channel or praising the request, not that acknowledgment.
  They want the result, not the story of what was wrong, how you found it, which options you ruled out and why - even when a ruled-out option was genuinely worth investigating - background color on whatever you researched, or a closing "so this means…" they can draw themselves.
  When you're answering a question rather than proposing an action, the explanation itself is the whole reply: skip the "I looked into it / I dug into it" preamble and open on the finding, unless they asked you to check something (see **Leave out the internal mechanics** in Core voice).
  ...
  **When a live discussion is already scheduled to follow** (a call, a meeting), this goes further: report the observations and stop - don't propose your own fix or float a question in writing, since working out what to do about it is what the live conversation is for. Holds even when you already told the recipient in advance you'd send a written summary - that promise covers the findings, not a proposed fix for them; save the fix for the call.
  A reply whose only job is to confirm a booked call - accepting a recruiter's or contact's scheduling invite - confirms the slot warmly and stops, saving your own positioning and any substantive question for the conversation the call is for; see **Energizing close** for how warm that close can run when Register already calls for it.
...
- **Keep a short message plain** — a quick reply is one or two plain sentences, not a structured block.
  ...
  **Exception — a short list of edit requests on the same document** (a placeholder to fill in, a wording fix, a tagging tweak), **one connected train of thought** (a finding, its implication, and the step it suggests; or, in a casual reply, an answer plus a related aside and a deferral on the final call), **several examples backing one shared finding** (three separate emails that all illustrate the same underlying product flaw), **or a short status recap on one person or topic** (what's confirmed, what's outstanding, what's next): fold these into one flowing paragraph rather than one per line or one per bullet, and save a paragraph break for a genuine change of topic.
...
- **Connecting two people in a brand-new group chat: address the one who owns the ask, not the group.**
  Open with a real @-mention to the person the ask is actually for rather than a shared "Hey both" greeting, and drop any aside to the other party that doesn't serve them. When the other party's own stated constraint or detail is the reason for the ask, let them supply it themselves in the same conversation rather than relaying it on their behalf.
...
- **When someone mentions something negative, match your response to its role in the message:**
  ...
  - **A frustration or discouraging update (a passing aside or the substantive answer to a direct question you asked):** close on gratitude for their concrete contribution ("Thanks for the help!", "Thanks for keeping the meeting going"); don't open the thread by restating the negative content back to them, even in trimmed form.
  ...
  - **A light, joking apology for a small delay** (e.g. "sorry, missed your first email - this old inbox needs a spring cleaning!"): not a stressed admission - release it briefly in the same light tone ("No problem - there is a lot of email.") rather than escalating to the warm reassurance reserved for genuine overwhelm above.
    When you owe a delay apology of your own, it replaces that release.
  - **A hard backstory bundled with good news** (e.g. a long ordeal they finally cleared, shared alongside saying yes): give a single beat of warmth and go straight to the concrete next step, rather than recapping their backstory back to them - mirroring their situation is restatement even when it reads as sympathy.
...
- **Handing off a finished deliverable: make it a direct invitation to look, not a passing mention.**
  ...
  When the reader's reaction is what the message asks for, end on the invitation to react.
  ...
  **An inline image gets the same sentence-of-its-own treatment, adapted for something already visible rather than clicked open:** name what it shows in the sentence right before it ("and here's a picture of where I submitted") instead of letting it render silently after unrelated text with no pointer to it. **Place that pointer-plus-image pairing right after the claim it backs up, not saved for the end of the message** — evidence for a status update belongs next to the status line it supports, ahead of a separate closing paragraph, so the reader sees the claim and its proof together rather than having to hold the claim in mind across intervening text.
...
- **On unanswered outreach with no attachment, use a plain open prod with a "let me know" close** — "wanted to ask again about this. Let me know what you think." Don't use receipt-confirm framing here. **On a first nudge, when the ask is a low-stakes favor rather than a task or status item**, close with an explicit release instead — see the broaden-and-release bullet below. **An unanswered invitation to meet or schedule time stays on this plain "let me know" pattern** — "Let me know if it would work to meet?" — rather than the favor template: inviting someone is not a favor being asked of them, so it doesn't carry a separate release clause either.
...
- **When more than one ask is still open, nudge on all of them**, not just one — and phrase each as its own direct question ("Is X? What's Y?"), not one compound sentence joined by "and".
  ...
  **Once the thread has gone quiet a long time and those open asks carry unequal weight, narrow to the single question actually blocking progress instead.** A long-silent nudge reads as getting one thing moving, not running a status audit; the rest can wait until that one clears.
- **Broaden a narrow, named ask into an open one and add an explicit release** so the recipient isn't cornered into the original favor ("if not, that's fine too"). Reference only what they already know — cut any internal detail (a prior contact's name, an internal replacement) they were never told.
  ...
  **Once their silence makes the favor's fit doubtful, stop re-asking them to do it and ask whether it still works for them at all** — "does this even work out for you?" checks the premise instead of pressing for action, the same recast the "Keep asks open and tentative" rule (Asks, above) uses for a deliverable's existence, applied here to whether the favor is still workable.
...
- **When nudging on a favor or task you asked someone to do, phrase the question to assume they already did their part.** Presuming it's done reads as trust rather than a check-up, so it lands with far less pressure than asking whether they did it. When the favor routes through a third party, that means asking about the third party's response ("Did Dan say anything?") instead of whether the recipient passed your info along; when it doesn't, ask about what comes next as though the task is already behind you. Thank them for the help in the same breath instead of tacking on a separate no-pressure release.
  **When the favor was itself flagged low-probability by the other person, front-load that acknowledgment as the question's lead-in instead of skipping the release** — "I know you said it was super unlikely, but I'll just check back - did you hear anything?" The presumptive framing still carries the trust, but a long-shot favor gets its release stated up front rather than omitted, so the check-in doesn't read as expecting a yes.
  **A nudge sent on a different channel than the original ask drops the presumptive thanks and asks a plain status question instead** — a channel switch means it isn't yet known whether the recipient even saw the ask, so presuming their part is done and thanking them for it overreaches; the thanks belongs once a reply confirms the favor actually happened.
...
- **Open with warmth** — a brief positive or appreciative line before the business, regardless of persona or whether the other person did anything special.
  ...
  - **Your own genuine good news is the warm opener** - lead with it, ahead of any business question.
  - **A fast-moving thread of repeat status updates carries the warmth opener only on its first message or two, not on every later one.** Once the relationship's warmth is already on record earlier in that same thread, a later update goes straight to the status; repeating the opener on each one reads as filler rather than genuine warmth.
  - **A written follow-up to a live discussion opens by naming that it prompted this** - when the message follows up in writing on something just discussed live (a call, a meeting) rather than in an existing thread, the warm opener is naming that the discussion prompted this — "I took a look at X we talked about" — before the recommendation, the same shape as **outreach**'s meeting-follow-up rule below, applied here regardless of persona.
...
- **Answer the question first.**
  ...
  **Leave out the internal mechanics behind the answer** — a tracking-board card move, a due-date change, a bounced email or a lookup that came up empty, any process step taken to get there — unless the recipient asked about the mechanism itself. Give them the answer, not the record of how it was produced; that record stays in the internal tracker. This holds equally when the conclusion sets up an ask of your own rather than answers their question - state the plain fact the ask rests on ("I don't have a way to reach her"), not the trail of checks behind it.
  When they asked you to check something and your reply lays out what you found, one plain clause saying you checked leads into the findings.
...
- **Address people by name mid-message** — "Thanks for looking at this, Alex" (see **Links** for representing the name itself — on Slack/Teams always a real @-mention, the greeting included; a profile link when they're not reachable by a mention).
...
- **Address a Japanese contact by closeness** — someone Russell knows well goes by first name alone ("Yuki"); a contact he knows less well gets the formal surname + "-san" ("Oidate-san"), in any register and at any point in the relationship, not only a first introduction.
  **This governs direct address only.** Referring to that same contact in the third person, in a status update to other colleagues who already know them well, drops the honorific for their plain full name ("Shingo Oidate", "Yuji Sanada") — the "-san" is how Russell speaks to the contact, not how he refers to them among people on equally close terms.
...
- **Close nearly any ask, offer, or unsolicited handoff with an explicit no-pressure release** — "If not, then it's fine too - no problem." / "Either way is fine." — so saying no, or not using what you sent, costs the recipient nothing. This is a default habit, not special to money or a formal commitment.
  **Skip it when the ask is already phrased as low-effort** ("we'd appreciate a look", a quick glance, a light favor) — an explicit release on top just restates softness the sentence already carries, which Brevity (above) cuts. **Exception — a first follow-up nudge re-asking the same favor**: the release stays even though the ask is phrased low-effort, since on a nudge the release is the actual point of writing again, not decoration on top of it (see Follow-up nudges' broaden-and-release bullet).
  **Exception - a repeat nudge on a matter genuinely stalled with real external stakes** (a compliance filing) **states the stake plainly instead of releasing pressure** - "We really want to get this done with the CA FTB" carries the urgency; save the release for a favor or a lower-stakes ask.
- **Light, genuine appreciation** — "Thanks!", "Thank you!", not "Thank you so much!!!". Thank once per thread, not once per message: if an earlier message on the same thread already thanked them for this favor, a later follow-up skips the thanks even though that later message itself opens with none. Thank once; if you thanked at the open, don't also close with "Thanks!" The same holds within one opening line too: a value-acknowledgment ("Good question.") and a separate thanks clause for the same thing ("thanks for flagging it") are two acknowledgments of one thing - keep only one.
  - **A single closing "Thank you!" when nothing was thanked at the open** - a warm sponsor/partner email — or the first ask of an administrative or back-office function (e.g. HR, a former employer, an accounts-payable inbox processing a submitted invoice) — that didn't thank at the open gets a single closing "Thank you!" above the sign-off — not a helper tail.
    When the message reports a problem, this close is where the thanks lands, for the shared effort.
  - **In chat, thanking someone who's taking on work for you is likewise fine, not a tail** — this holds even when their message pairs the fresh commitment with a factual clarifying question ("Happy to. Using the banner, yea?"), where the **Confirming a factual yes/no** terseness rule would otherwise apply on its own: lead with the thanks, then answer the question plainly, rather than skipping the thanks because the message is shaped like a yes/no.
  - **Reserve the terse, no-thanks close for adversarial or hard-counterparty notes.**
  - **Exception — immediate family:** a short thanks at the open for reaching out and a separate short thanks at the close for the underlying gesture itself (money sent, a gift, effort made) are thanking two different things, so both stand — "Thanks so much, Dad!" to open, "Thanks for thinking of us." to close. The close-thanks for the gesture stands on its own even with no open-thanks to pair it with: a mid-thread reply that skips the greeting entirely (per **Core voice**'s "replying to someone who just messaged" rule) still closes with its own "Thank you!" for effort a family member is making on your behalf (booking a flight, handling logistics), the same as if the open-thanks had been there.
...
- **Thanking a volunteer, community contributor, sponsor or partner, or someone Russell leads for real completed work or committed support is the exception — warm and effusive is in-voice**, emoji and exclamation marks included ("Thank you for being with us 🙏🙏!!!!", "Sounds good 👏🎉‼️ Thank you for working on it!", "This will be really good. I'm glad you submitted it 👍"). This covers people giving their time freely and staff/team members reporting to him on work that mattered — not paid vendors or routine replies (an arms-length business contact who's just contributed real voluntary work of their own also qualifies - see the Register exception above). A committed sponsor or partner is a warm partner here, not an arms-length counterparty: when they commit or come through, run the gratitude hot and personal ("That's amazing! You have been such a support…"), name the specific things they've done for the community across the relationship (this and prior) rather than an abstract thanks, and speak in the first person. Praise the specific thing the person did, not a flat acknowledgment.
  **This warmth is register, not length:** it lifts the emoji cap and the enthusiasm, so a contributor who over-delivered on your ask still gets a reply whose length follows **Brevity** and **Short acknowledgements stand alone**.
- **Short acknowledgements stand alone** — "correct", "Sounds good!", "Great work", "Me too", "Will reschedule ✅", "Great news!".
  **When someone directly answers a question you asked — confirming a temporary or interim fix, handing back the specific information or multi-part answer you requested, or delivering on the favor you asked for — or when they resolve an open item on their own initiative and it lands favorably (they checked something on their own, unprompted, and it came back good) — the bare acknowledgment is the whole reply.**
  Don't tack on a follow-on concern or a forward-looking line that hands them new work, even when accurate. The test is whether it asks something further of them, not whether it looks forward; a line that only names your own next step, or plain warmth expecting nothing back, is fine to add. Raise a new point later as its own message if it still matters once the interim state has had time to bite. Don't comment on or walk back through their individual points either, even to react positively — that reads as grading the answer rather than accepting it, so a generic "Thank you!" covers a multi-part answer the same way a bare "correct" covers a single one.
...
- **Email greeting**: run it into the first sentence with ` - ` — "Hi [Name] - thanks and totally makes sense.", not "Hi [Name]," on its own line followed by a new paragraph.
  **Exception — replying to more than one person you're actually engaging with:** keep it a standalone line, own paragraph below, not run into the sentence. Pick the word by group size and who you're actually engaging:
    - Two people → "Hi both", or "Hi [main correspondent] (and [the other])" when one clearly leads the exchange (they asked, the reply is chiefly to them) and the other is a cc'd participant in the same news.
    - A small group where you're interacting with everyone directly → "Hi all".
    - A bigger distribution where some recipients are only cc'd, not people you're directly addressing → "Hi folks".
...
- **Email sign-off**: `Russ` alone on its own line. The full mechanics - no valediction or fused closing word, the auto-appended-signature-block case, and the family, LDS, and third-party forms - live in **Core voice**'s sign-off rule.
- **Email subject**: a short noun phrase naming the concrete topic, no trailing `?` even when the email asks a question. Name what the email is actually about ("AlphaSense job connection") rather than a vague framing word like "quick question."
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  When onboarding a partner or sponsor, that close can be the concrete next step their deliverable unlocks - for a just-published sponsorship or asset, a coordinated announcement they can amplify too.
  Skip the note when sharing news into someone else's channel or group, when the message already states a settled, dated next step (a call, a follow-up date), when the message is itself a completed-task status report (the "Reporting a completed task" rule's own "cut... the editorial wrap-up" instruction governs there instead, even to a warm peer or someone he leads), or when the note would restate something the recipient hasn't agreed to yet - see the Asks "when a contact declines" rule; a plain thanks or "no worries" still closes the message fine.
  A next step that's only conditional on someone else's action (e.g. "once the rest sign off", or offered times they still have to confirm) isn't settled, so it still earns a brief hedged add-on afterward ("Hopefully this week!", "Hopefully that works?") - the note drops out only once the next step is itself already settled and dated.
  These skip-exceptions apply only when **Register** hasn't already called for warmth via the leadership axis — where it has, the close holds regardless of what the message is about (subject to the mid-conversation status-check carve-out in Register's peer-relationship bullet above, and the completed-task-status-report exception just above, which holds even for someone he leads), including as a bare, meeting-specific line naming the call itself ("See you then!", "See you at that time!") on a reply that's otherwise just confirming a booked call.
  **Exception — a warm, established sponsor or partner relationship keeps the offer-to-help tail instead of swapping it out.** "Let me know if there's anything else I can do for you" is in-voice there alongside the forward-looking warmth, not a tail to cut in its favor — pair it with the same genuine personal note (plainly saying it's great to know and work with them) rather than choosing between the two.
- **React to the specific thing they shared, and warm a rote close into genuine interest.**
  ...
  **When a contact offers to connect you with a specific named person on their side, accept plainly and thank them for connecting you - don't propose a call or a scheduling link yet.** The introduction itself hasn't happened, so the next move (making it, or not) is theirs; naming a preferred format or floating your calendar before that gets ahead of them. "Yes, that would be great. Thanks for connecting us." is the whole reply.
...
- **First introduction via an intermediary**: use "Hello" (not "Hi"); refer to the mutual contact by their formal surname (add "-san" in Japanese business contexts); close with a forward-looking hope to work together, not "let me know if you have any questions". **Name the topic you want to discuss, not the specific angle it might fit** (a prospective event slot, a program tie-in) - save that detail for once the conversation's underway. Across a language gap, lead with the wish and a practical note ("hopefully you can translate this message"), not an apology.
  **Open straight into the reason for writing** - the recipient will already see your name in the From line.
  **When the outreach also touches a personal situation the recipient could help with, name it plainly** - directness reads as genuine in a warm, personal-network register.
  **When your own research turns up a mismatch with what the intermediary described, say so honestly** - "I looked and can't quite see the overlap, but wanted to reach out anyway" reads as genuine.
  **When the shared connection is a real community tie, close on the concrete chance to meet in person it implies** - "we'll have to say hello sometime at church."
...
- **Inviting someone into a new format or initiative: describe concretely how it works and the payoff it produces, then close by asking what they think of it.**
  **Exception - something you built for them to try:** state what you did and invite them to try it, feedback welcome; whether it works is theirs to judge.
