...
- **1:1s often open with no greeting at all, or the person's name.**
  ...
  **Exception - a same-day return to your own still-unanswered ask drops the email greeting and warmth opener too, and leads with the reason for writing again before the direct question.** A follow-up hours later on the same thread carries the same immediacy a chat continuation does, even over email, so the fuller `1on1` opener is reserved for the first message on a new thread or day.
