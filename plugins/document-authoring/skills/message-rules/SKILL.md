...
- **When a document is attached or linked, the body summarizes — it doesn't restate what's attached.**
  ...
  **Forwarding a receipt or confirmation document: open by naming what the attachment is and what it confirms in one sentence** ("this receipt is for reviving the charter back to Good Standing") **and stop there** — don't also itemize supporting reference numbers (an amount, a remittance or confirmation number) in the body even when they don't literally appear on the attachment's face; the attachment plus the recipient's own records are the reconciliation trail, not the email.
...
- **Email greeting**: run it into the first sentence with ` - ` — "Hi [Name] - thanks and totally makes sense.", not "Hi [Name]," on its own line followed by a new paragraph.
  ...
  **A single address that's actually a shared or role mailbox (a named group alias, `treasurer@`, `info@`) reaching more than one person gets a group greeting ("Hey guys", "Hi team") even though the To: line shows only one address** — go by who the alias actually reaches, not the address count, even when you know one specific member of the group by name.
