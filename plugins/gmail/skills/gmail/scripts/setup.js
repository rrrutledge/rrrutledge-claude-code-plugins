...
// imapflow) into a stable per-user location so every gmail script can resolve them, regardless of which
// directory it runs from. googleapis is the Gmail REST API client; google-auth-library builds the OAuth
// client both gmail.js and filters.js authorize through; nodemailer's MailComposer builds outgoing MIME (no
// SMTP transport); mailparser reads fetched RFC822 source; marked renders Markdown bodies; imapflow backs
// gmail.js's narrow --list-inbox-imap path the drainer poller uses (see gmail.js's file-header comment).
...
if (present('googleapis') && present('mailparser') && present('marked') && present('nodemailer')
    && present('google-auth-library') && present('imapflow')) {
