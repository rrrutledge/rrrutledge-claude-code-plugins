...
Reserve a launched session (a separate background session) for fan-out: many independent browser tasks with no shared login and no mid-run gate, the way the drainer runs one headless worker per inbox item.
